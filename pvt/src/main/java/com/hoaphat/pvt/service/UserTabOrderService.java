...
public class UserTabOrderService {
    @Autowired private IUserTabOrderRepository repository;
    public List<String> getOrderedTabs(String managerUsername) {
        return repository.findByManagerUsernameOrderByDisplayOrderAsc(managerUsername).stream().map(UserTabOrder::getAccountName).collect(Collectors.toList());
    }
    public void saveOrder(String username, List<String> tabs) {
        ...
        for (int i = 0; i < tabs.size(); i++) {
            ...
            u.setManagerUsername(username); u.setAccountName(tabs.get(i)); u.setDisplayOrder(i);
            repository.save(u);
        }
    }
}